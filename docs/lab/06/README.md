...
Реализовать функционал отзывов на мероприятия с помощью Cassandra и Redis.
...
> 💡 Учитывайте, что в Cassandra нет оператора `OFFSET`
...
> ⚡️ DDL — на ваше усмотрение. Рекомендуется добавить `created_at` по **убыванию** в ключ кластеризации
...
```plaintext
event:{event_title_md5_hash}:reviews
```
...
```plaintext
event:ba80405c3ebccb9cb99791b47c2487f9:reviews
```
...
округлённый до одного знака после запятой по правилам математического округления

### Конфигурация

Добавьте TTL для данных по отзывам мероприятий в Redis
...
**Q: Добавляем новые ноды в кластер Cassandra?**  
A: Нет, в рамках лабораторной работы мы не будем масштабировать кластер Cassandra,
так как каждый инстанс потребляет много ресурсов
и может не запуститься на слабых машинах и вызвать проблемы с АТ.
Но в реальной жизни, конечно же, нужно масштабировать Cassandra
...
**Q: Может ли пользователь оставить отзыв на одно и то же мероприятие, но на другое дату и время в расписании?**  
A: 🤡 На текущий момент — **да**.
Это связано с тем, что проверка уникальности отзыва осуществляется
только по комбинации `event_id`, без учёта времени проведения мероприятия из расписания.
Таким образом, пользователь может оставить несколько отзывов на одно и то же мероприятие, если оно проходит в разное время.
Это поведение можно считать **недостатком бизнес-логики**, который был выявлен на позднем этапе разработки.
Следовало бы учитывать идентификатор сеанса (например, `schedule_id`) при создании отзыва,
чтобы привязывать отзыв к конкретному показу.

**Q: Когда обновлять Redis данным по отзывам?**  
A: При каждом успешном добавлении и при каждом успешном обновлении отзыва через соответствующие эндпоинты. TTL берется из env-переменной `APP_EVENT_REVIEWS_TTL`.
