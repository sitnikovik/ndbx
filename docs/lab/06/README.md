...
A: При каждом успешном добавлении и при каждом успешном обновлении отзыва через соответствующие эндпоинты.
...
**Q: Как безопасно добавить новые ноды в кластер Cassandra в Docker?**  
A: Первая нода (seed) должна быть **полностью готова** до запуска остальных, иначе вторичные ноды могут не подключиться и завершить работу с ошибкой из-за проблем с gossip.  
Рекомендуется:

- Использовать улучшенный `healthcheck` с `nodetool statusbinary`
- Запускать вторую и третью ноды **только после подтверждения**, что seed-нода в статусе `UN` (Up & Normal).
- При необходимости — управлять порядком старта через sh-скрипт, который проверяет готовность первой ноды перед запуском остальных.
- Не использовать `sleep` как единственный механизм ожидания
