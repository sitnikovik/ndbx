...
### Дислайк на мероприятие

Реализуйте новый эндпоинт `POST /events/{event_id}/dislike`,
который ставит дислайк пользователя на конкретное мероприятие.
...
> ⚠️ Все эндпоинты по мероприятиям должны возврщать объект `reactions`,
...
`event_reactions` - название таблицы лайков на мероприятия

**Схема**:

- `event_id` *text* - идентификатор мероприятия в MongoDB
- `user_id` *text* - идентификатор организатора мероприятия (пользователь) в MongoDB
