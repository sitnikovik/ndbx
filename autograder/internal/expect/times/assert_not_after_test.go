...
func TestAssertNotAfter(t *testing.T) {
	t.Parallel()
	type args struct {
		expected time.Time
		actual   time.Time
	...
	tests := []struct {
		...
		{
			name: "not eq different timezones",
			args: args{
				expected: timex.MustParse(time.RFC3339, "2024-06-01T00:00:00Z"),
		...
		{
			name: "eq different timezones but same time",
			args: args{
				expected: timex.MustParse(time.RFC3339, "2024-06-01T00:00:00+02:00"),
				actual:   timex.MustParse(time.RFC3339, "2024-06-01T00:00:00Z"),
			},
