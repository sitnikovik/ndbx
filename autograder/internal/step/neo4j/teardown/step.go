...
const (
	...
	Description = "Tears down the Neo4j environment for tests."
)

// neo4jClient defines the interface for interacting
// with Neo4j to perform the teardown job.
type neo4jClient interface {
	// DeleteAll removes all nodes and relationships from the database.
	DeleteAll(ctx context.Context) error
}

// Step represents the Neo4j teardown step for tests.
type Step struct {
	// cli is the Neo4j client used to perform the teardown job.
	cli neo4jClient
}
