...
// Run performs the HTTP check-in step by sending a POST request to the specified URL,
// validating the response, and extracting the session cookie
// and comparing it with the session cookie from the previous step.
func (s *Step) Run(
	_ context.Context,
	vars step.Variables,
) error {
	...
	if !ck.HttpOnly {
		return errs.Wrap(
			errs.ErrMissedCookie,
			"expect %s cookie to have HttpOnly flag",
			log.String(ckname),
		)
	}
	if ck.MaxAge <= 0 {
	...
	err = strings.AssertEquals(
		vars.
			MustGet(consts.SessionName).
			AsString(),
		sid,
	)
	if err != nil {
		return errs.Wrap(err, "session validation")
	}
	return nil
}
