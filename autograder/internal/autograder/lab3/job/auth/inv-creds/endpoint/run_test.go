...
import (
	...
	"github.com/sitnikovik/ndbx/autograder/internal/autograder/lab3/job/auth/inv-creds/endpoint"
					...
					httpxfk.WithPostJSON(
						func(_ string, _ io.Reader) (*http.Response, error) {
							return &http.Response{
								...
								ContentLength: int64(len(`{"message":"invalid credentials"}`)),
								Header: http.Header{
									"Set-Cookie": []string{
					...
					httpxfk.WithPostJSON(
						func(_ string, _ io.Reader) (*http.Response, error) {
							return &http.Response{
								...
								ContentLength: int64(len(`{"message":"invalid username or password"}`)),
								Header: http.Header{
									"Set-Cookie": []string{
										"X-Session-Id=0123456789abcdef0123456789abcdef; HttpOnly; Max-Age=3600; Secure=true",
									},
